...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      return queue.textChannel.send("🚫 Muzik Bitti.").catch(console.error);
    }
    ...
    try {
      var playingMessage = await queue.textChannel.send(`🎶 Çalınmaya Başlandı: **${song.title}** ${song.url}`);
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        case "⏭":
          ...
          queue.textChannel.send(`${user} ⏩ Müziği Atladı`).catch(console.error);
          collector.stop();
          break;
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Tekrar Şuan ${queue.loop ? "**Açık**" : "**Kapalı**"}`).catch(console.error);
          break;
